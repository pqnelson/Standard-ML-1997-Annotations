...
\begin{puzzle}{Strength of metatheory?}
These ``semantic objects'' live in some kind of metatheory or
metalanguage. How strong must this metalanguage must be? Presumably
HOL suffices (so something like Zermelo set theory also suffices), but
would a weaker metatheory work?
\end{puzzle}

\begin{remark}{Definition describes how to produce trees}
The Definition describes how to produce (what Milner and Tofte~\cite{milner1991commentary} call)
`elaboration trees' using the static semantics, and `evaluation trees' 
using the dynamic semantics. This may be surprising to readers familiar
with type theory having learned it from, e.g., TAPL where evaluation is
just a judgement form described using inference rules.

So be forewarned: the static module chapter describes how to form these
elaboration trees, the static core chapter describes one particular form
of a ``structure declaration''. The dynamic chapters describe analogous
results with the evaluation trees. The last chapter describes how to
combine the two for a program.
\end{remark}

\begin{puzzle}{Related static semantics to Hindley--Milner type system}
The Hindley--Milner type system describes a type theory for some
polymorphic simply-typed lambda calculus. Some references (like
Benjamin Pierce's TAPL) use a typing judgement with ``constraints'' to
describe the Hindley--Milner type system. (I believe the original
paper had the typing judgement look like $\Gamma\vdash t:T,S$ where
$S$ is a substitution used in unifying expressions. Pierce turned $S$
into a constraint.) How can we extend this traditional presentation of
Hindley--Milner to Standard ML's static semantics? Is this even the
...
\begin{comment}{Type names internally track \texttt{datatype} type operators}
Just for clarity's sake, the $t\in\TyNames$ are metasymbols which are
freshly generated whenever a \texttt{datatype} declaration is encountered. This
technically differs from a type operator (which is constructed whenever
a \texttt{type} alias is encountered).

But we can still meaningfully talk about the ``arity'' of a type name.
The Definition allows for eta-equivalence, so $\Lambda\alphak.t$ is
replaceable by $t$.
\end{comment}
...
\begin{definition}{Type of special constants}\label{defn:scontype}\index{Type!of special constants}\index{Special constant!Type}\index{type scon@$\scontype(\scon)$}\index{$\scontype(\scon)$}
...
The domain and range of a finite map $f$ are denoted
``$\Dom f$'' and ``$\Ran f$'', respectively.
...
commutative\footnote{Except for ordinals, where $1+\omega=\omega$ but $\omega+1=\omega\cup\{\omega\}$.}, but $f+g\neq g+f$ when
...
\begin{comment}{Universal quantifier}
The universal quantifier $\forall$ has a similar meaning as used in
System F, it appears. But what TAPL calls ``type application'' is
handled implicitly rule~\eqref{rule:static-core:value-variable}.
\end{comment}
...
\begin{comment}{Injections initialize all other components to empty set}
...
A type scheme $\tych=\forall\alphak.\tau$ \define{generalizes} $\tych'=\forall\beta^{(\ell)}.\tau'$,
written $\tych\succ\tych'$, if $\tych\succ\tau'$ and $\beta^{(\ell)}$ contains
...
\begin{comment}{Type schemes should be thought of as ``most general type''?}
I think that type schemes should be considered as the ``most general
type'' for a given expression $e$. If that expression $e$ is used in a
situation which requires a ``less general type'' (i.e., $e$'s most
general type successfully generalizes the situation's required type),
then it typechecks alright.
\end{comment}
...
\begin{comment}{Non-expansive expressions used in closure of value declarations}
We should note that the notion of ``non-expansive expressions'' only
matters in the definition of the Closure of a value environment relative
to a value binding declaration~\zref{defn:closing-value-environment}.
This occurs in let-expressions, which is the value restriction rule.

In effect, this is how let-polymorphism is implemented in Standard
ML. I should work through, as an example, how the static semantics
determines the typing of the program:
\begin{verbatim}
fun foo (x : string) : unit =
  let
    fun id y = y
  in
    print (Bool.toString(id true));
    print (id "\nmore stuff\n");
    print x
  end;
\end{verbatim}
...
\begin{comment}{Abs(\TE, \E) used for abstype declarations only}
...
\begin{remark}{Type operators enter here}
This is the only way for type operators to appear, it's through type
binding rules.

Datatypes introduce new ``primitive notions'', as it were: a ``type name''
which acts like a tagged pair. This resembles a type operator, but does
not expand the ``tag'' part at all. The Definition makes a distinction
between type operators [``type functions''] and ``type names'', but this
seems more like a distinction without a difference.
\end{remark}
...
\begin{inference-rule}{Constant pattern}
...
\begin{inference-rule}{Record type}
...
\begin{comment}{Additional impositions}
